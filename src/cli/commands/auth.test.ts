// Tests for the `gmail auth` CLI driver (runAuthCommand).
//
...
// Mock the OAuth flow so runAuthCommand doesn't actually start a loopback
...
beforeEach(() => {
  ...
  // Capture writes — runAuthCommand prints to both streams.
...
describe("runAuthCommand --print-json", () => {
  it("prints {GMAIL_OAUTH_KEYS_JSON, GMAIL_CREDENTIALS_JSON} to stdout (disk-keys path)", async () => {
    ...
    // (runOAuthFlow is mocked, so the port is never bound; but runAuthCommand
