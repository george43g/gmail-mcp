...
const create_filterEval: EvalFunction = {
  ...
  run: async () => {
    const result = await grade(
      openai("gpt-4"),
      "Create a filter that automatically labels emails from newsletter@company.com with 'Newsletter' label and archives them (skips inbox).",
    );
    return JSON.parse(result);
  },
};
...
const create_filter_templateEval: EvalFunction = {
  ...
  run: async () => {
    const result = await grade(
      openai("gpt-4"),
      "Create a filter using a template to automatically handle all emails from notifications@github.com by labeling them as 'GitHub' and archiving them.",
    );
    return JSON.parse(result);
  },
};
...
const list_filtersEval: EvalFunction = {
  ...
  run: async () => {
    const result = await grade(
      openai("gpt-4"),
      "Show me all my current Gmail filters and their configurations.",
    );
    return JSON.parse(result);
  },
};
...
const filter_managementEval: EvalFunction = {
  ...
  run: async () => {
    const result = await grade(
      openai("gpt-4"),
      "I want to create a filter for managing marketing emails. Filter emails containing 'unsubscribe' in the body, label them as 'Marketing', mark them as read, and skip the inbox.",
    );
    return JSON.parse(result);
  },
};
